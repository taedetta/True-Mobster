...
router.post('/register', async (req, res) => {
  try {
    ...
    await db.run(`INSERT INTO players (user_id, display_name, energy, max_energy, stamina, max_stamina, health, max_health, referral_code)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, displayName || username, BASE_STATS.maxEnergy, BASE_STATS.maxEnergy, BASE_STATS.maxStamina, BASE_STATS.maxStamina, BASE_STATS.maxHealth, BASE_STATS.maxHealth, refCode]);

    await processReferral(id, referralCode);

    const token = signToken(id);
    res.status(201).json({ token, userId: id, username, displayName: displayName || username, referralCode: refCode });
  } catch (err) {
...
router.post('/login', async (req, res) => {
  try {
    ...
    const player = await db.get('SELECT display_name FROM players WHERE user_id = ?', [user.id]);
