...
import { MOB_ALLY_CONTRIBUTION } from '../../../shared/gameData.js';
...
export async function sendChatMessage(userId, channel, message) {
  ...
  const player = await db.get('SELECT display_name, crew_id FROM players WHERE user_id=?', [userId]);
  ...
  await db.run(
    'INSERT INTO chat_messages (channel, user_id, display_name, message) VALUES (?, ?, ?, ?)',
    [ch, userId, player.display_name, body],
  );
...
export async function getChatMessages(channel, crewId, limit = 50) {
  let ch = channel;
  if (ch === 'crew') {
    if (!crewId) return [];
    ch = `crew:${crewId}`;
  ...
  return db.all(
    'SELECT id, channel, user_id, display_name, message, created_at FROM chat_messages WHERE channel=? ORDER BY created_at DESC LIMIT ?',
...
export async function sendPrivateMessage(fromId, toUsername, subject, body) {
  ...
  await db.run(
    'INSERT INTO private_messages (from_id, to_id, subject, body) VALUES (?, ?, ?, ?)',
    [fromId, to.id, sanitize(subject).slice(0, 80), text],
  );
  return { toId: to.id };
}

export async function getPrivateMessages(userId, box = 'inbox') {
  if (box === 'sent') {
    return db.all(
...
export async function getMobAllies(userId) {
  return db.all(
    `SELECT p.user_id, p.display_name, p.level, p.mob_size, p.referral_code, ma.added_at
