...
export default function ChatPage() {
  ...
  }, [tab, state?.crew?.id, state?.locale]);
  ...
  useEffect(() => {
    ...
    const onMsg = (msg) => {
      const enriched = {
        ...
        source_locale: msg.source_locale || 'en',
        translated: false,
      };
      if (tab === 'world' && msg.channel === 'world') setMessages((prev) => [...prev, enriched].slice(-100));
      if (tab === 'crew' && msg.channel === 'crew') setMessages((prev) => [...prev, enriched].slice(-100));
    };
          ...
          <div className="space-y-2 flex-1 overflow-y-auto max-h-64">
            ...
            {pms.inbox?.map((m) => (
              <div key={m.id} className={`card text-sm ${!m.read_status ? 'border-mob-gold/30' : ''}`}>
                <Link to={`/player/${m.from_id || ''}`} className="text-mob-gold text-xs font-semibold hover:underline">{m.other_name}</Link>
                <p className="font-semibold">
                  <TranslatedText
                    text={m.subjectDisplay || m.subject || '(no subject)'}
                    original={m.subjectOriginal || m.subject}
                    translated={m.translated && !!m.subject}
                ...
                <p className="text-gray-300 mt-1">
                  <TranslatedText
                    ...
                    original={m.bodyOriginal || m.body}
          ...
          <div className="flex-1 overflow-y-auto max-h-80 space-y-2 card min-h-[200px]">
            {messages.map((m) => (
              <div key={m.id} className={`text-sm ${m.user_id === state.user_id ? 'text-right' : ''}`}>
                ...
                <p className="text-gray-200 bg-mob-bg/50 rounded-lg px-2 py-1 inline-block mt-0.5">
                  {m.user_id === state.user_id ? (
                    m.message
                  ) : (
                    <TranslatedText
                      text={m.messageDisplay || m.message}
                      original={m.messageOriginal || m.message}
                      translated={m.translated}
                      sourceLocale={m.source_locale || 'en'}
                    />
                  )}
                </p>
              </div>
