  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {achievements.map((a) => (
          <div key={a.id} className={`card ${a.completed ? 'border-green-800/40' : ''}`}>
            ...
            {a.completed && !a.claimed && (
              <button className="btn-primary text-xs w-full mt-2" onClick={() => claim(a.id)}>Claim</button>
            )}
            {!a.completed && <p className="text-xs text-gray-600 mt-1">Not yet unlocked</p>}
          </div>
        ))}
