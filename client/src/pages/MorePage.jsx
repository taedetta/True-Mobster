  ...
  return (
    <div className="space-y-4">
        ...
        <div className="card border-red-800/50">
          ...
          {inJail && (
            <>
              ...
              <button className="btn-primary w-full text-sm mb-2" onClick={() => action('/safehouse/bail', {}, 'Bailed out!')}>
                Post Bail
              </button>
            </>
          )}
          {iced && (
            <p className="text-xs text-cyan-300">Iced until {new Date(state.iced_until).toLocaleTimeString()}</p>
          )}
          <div className="flex gap-2 mt-2">
            {[1, 4, 8, 24].map((h) => (
              <button key={h} className="btn-secondary text-xs flex-1" onClick={() => action('/safehouse/ice', { hours: h }, `Iced for ${h}h`)}>
                Ice {h}h
              </button>
            ))}
          </div>
      ...
      <div className="card text-xs text-gray-500 text-center">
        True Mobsters · {formatMoney(state?.money || 0)} on hand
      </div>
    </div>
  );
}
