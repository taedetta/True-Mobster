    ...
    <div className="space-y-4">
      {state.crew ? (
        <div className="card">
          ...
          {state.crew.treasury != null && (
            <p className="text-xs text-green-400 mt-1">Treasury: {formatMoney(state.crew.treasury)}</p>
          )}
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto">
            {state.crewMembers?.map((m) => (
              <div key={m.user_id} className="flex justify-between items-center text-xs py-2 border-b border-mob-border">
                <div>
                  <span className={m.role === 'leader' ? 'text-mob-gold' : ''}>{m.display_name}</span>
                  {m.role === 'leader' && <span className="text-mob-gold ml-1">👑</span>}
